...
언어 모델 평가, 특히 사후 학습 (post-training)에서의 평가를 이해하는 핵심은, 현재의 주요 평가 체계가 인기 있는 학습 모범 사례 및 목표를 반영한다는 점이다.
...
RLHF와 사후 학습의 평가는 초기 역사에서 몇 가지 뚜렷한 단계를 거쳐 왔다:
...
이 모델들을 더 개방적인 방식으로 사용하기 위해, 여러 예시들을 모델에 보여준 후 불완전한 구문 형태의 프롬프트를 입력했다. 이것이 퓨샷 (few-shot) 또는 문맥 내 학습 (ICL, in-context learning) [@brown2020language]이라 불리며, 당시에는 지시 미세조정이나 RLHF가 관여하지 않았다.
...
로그 우도 채점은 정확 일치에 필요한 질문-답변 형식이 부족한 사전 학습 (pretraining) 평가에서 더 일반적이며, 정확 일치는 사후 학습에서 표준이다 [@teamolmo2025olmo3].
...
파인튜닝된 언어 네트워크 (FLAN, Finetuned Language Net)는 현대 지시 미세조정의 전신으로서 특정 과제에 미세조정된 언어 모델이 학습하지 않은 제로샷 질문에도 일반화할 수 있음을 보였다 [@wei2021finetuned] (유사한 결과가 T0 [@sanh2021multitask]에서도 발견됨).
이것이 지시 미세조정 (IFT, instruction fine-tuning)의 등장으로, RLHF와 사후 학습의 중요한 선구자이다.
...
예를 들어, Olmo 3 프로젝트에서 저자들은 추론 모델 시대의 대부분의 사후 학습 평가가 평가 설정이 일정하게 유지될 때 표준 편차가 0.25에서 1.5 포인트 사이임을 발견했다 [@teamolmo2025olmo3] -- 더 큰 점수 변동은 다른 프롬프트나 샘플링 파라미터를 사용하는 것에서 올 수 있다.
...
사후 학습에서 데이터 형식이 어떻게 구성되느냐에 따라 모델은 평가 형식에 걸쳐 상당한 차이를 보일 것이다.
...
이것은 현대 언어 모델 학습 스택에서 사후 학습의 정교함에 의해 더욱 복잡해진다.
...
현재 언어 모델 관행 (즉, RLHF와 사후 학습에만 국한되지 않음)의 주요 문제는 학습에서 평가 데이터셋의 데이터를 의도적 또는 비의도적으로 사용하는 것이다.
...
다른 경우에는, 수학 문제의 단어는 같게 유지하고 숫자만 변경하는 것처럼 벤치마크에 매우 근접한 데이터로 학습된 것으로 발견된 모델들이 있으며, 이는 사후 학습 체계에서 특이한 동작을 초래할 수 있다. 예를 들어, 랜덤 보상으로 RL 학습 시 벤치마크가 향상되는데, 이는 모델이 특정 유형의 데이터 오염이 있는 경우에만 성능이 향상되어야 하는 인위적인 설정이다.
...
- 그들의 GPT-Neo-X 모델 인프라 위에 구축된 EleutherAI의 평가 하네스 [@gao2023evalharness] (좋은 GPT-3 시대 평가 설정과 구성을 포함함) [@gpt-neox-20b],
- OLMES [@gu2024olmes]를 기반으로 한 Ai2의 라이브러리,
- Stanford의 기반 모델 연구 센터(Foundation Models)의 HELM [@liang2023helm],
